...
if (!args[0]) return m.reply(`[ ⚠️ ] *Ingresa la cantidad de diamantes para agregar a su cuenta bancaria*`);
if (args[0] == '--all') {
let count = parseInt(users.limit);
users.limit -= count * 1
users.banco += count * 1
await m.reply(`*[ 🏦 ] Has agregado.*`);
return !0;
};
if (!Number(args[0])) return m.reply(`[ ⚠️ ] *Falta más diamante 💎*`);
let count = parseInt(args[0]);
if (!users.limit) return m.reply(`*Estas pobre, no tienes suficientes diamantes*`);
if (users.limit < count) return m.reply(`*Para saber cuanto dinero tienes usa el comando:* #bal`);
users.limit -= count * 1;
users.banco += count * 1;
await m.reply(`*[ 🏦 ] Has ingresado ${count} diamante al Banco*`)}
...
let count = parseInt(args[0]);
if (!user.banco) return m.reply(`Estas pobre, no tienes suficientes diamantes*`);
if (user.banco < count) return m.reply(`*Para saber. cuanto dinero tienes usa el comando:* #bal`);
user.banco -= count * 1
user.limit += count * 1
await m.reply(`*[ 🏦 ] Has Retirado (${count}) dinero del Banco*`)}

if (command == 'minar' || command == 'mine') {
const date = global.db.data.users[m.sender].lastmiming + 10000;
if (new Date - global.db.data.users[m.sender].lastmiming < 10000) return m.reply(`*${lenguaje.rpg.text9} ${msToTime(date - new Date())} ${lenguaje.rpg.text10}*`) 
...
const date = global.db.data.users[m.sender].lastmiming2 +100000;
if (new Date - global.db.data.users[m.sender].lastmiming2 < 100000) return m.reply(`*${lenguaje.rpg.text9} ${msToTime(date - new Date())} ${lenguaje.rpg.text10}*`)
...
let time = global.db.data.users[m.sender].lastwork + 1000000 //3600000
if (new Date - global.db.data.users[m.sender].lastwork < 1000000) return m.reply(`${lenguaje.rpg.text12}\n\n*${lenguaje.rpg.text9}* ${msToTime(time - new Date())} ${lenguaje.rpg.text13}`) 
